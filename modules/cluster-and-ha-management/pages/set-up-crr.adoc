...
* Install TigerGraph 3.9.10 or higher on both the primary cluster and the DR cluster *in the same version*.
...
# Start mirrormaker and begin cross-region replication
$ gadmin crr start
----
...
However, you can skip <<_enable_crr_on_the_dr_cluster, Step 3: Enable CRR on the DR cluster>> except running `gadmin crr start`, because CRR will have already been enabled and no need to restart services to make it effective.
...
3. Start CRR on the DR cluster(From TigerGraph 3.9.10, no additional restart is required to start CRR).
